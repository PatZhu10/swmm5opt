...
//  Local functions
//-----------------------------------------------------------------------------
EXTERN int    horton_setParams(THorton *infilParams, double p[]);
static void   horton_initState(THorton *infilParams);
static void   horton_getState(THorton *infilParams, double x[]);
static void   horton_setState(THorton *infilParams, double x[]);
static double horton_getInfil(THorton *infilParams, double tstep, double irate,
              double depth);
static double modHorton_getInfil(THorton *infilParams, double tstep, double irate,
              double depth);

static void   grnampt_getState(TGrnAmpt *infilParams, double x[]);
static void   grnampt_setState(TGrnAmpt *infilParams, double x[]);
static double grnampt_getUnsatInfil(TGrnAmpt *infilParams, double tstep,
              double irate, double depth, int modelType);
static double grnampt_getSatInfil(TGrnAmpt *infilParams, double tstep,
              double irate, double depth);
static double grnampt_getF2(double f1, double c1, double ks, double ts);

static int    curvenum_setParams(TCurveNum *infilParams, double p[]);
static void   curvenum_initState(TCurveNum *infilParams);
static void   curvenum_getState(TCurveNum *infilParams, double x[]);
static void   curvenum_setState(TCurveNum *infilParams, double x[]);
static double curvenum_getInfil(TCurveNum *infilParams, double tstep, double irate,
              double depth);
...
int horton_setParams(THorton *infilParams, double p[])
//
...
//
{
    ...
    infilParams->f0   = p[0] / UCF(RAINFALL);
    infilParams->fmin = p[1] / UCF(RAINFALL);

    // --- convert decay const. to 1/sec
    infilParams->decay = p[2] / 3600.;
    ...
    if (p[3] == 0.0 ) p[3] = TINY;
    infilParams->regen = -log(1.0-0.98) / p[3] / SECperDAY;

    // --- optional max. infil. capacity (ft) (p[4] = 0 if no value supplied)
    infilParams->Fmax = p[4] / UCF(RAINDEPTH);
    if ( infilParams->f0 < infilParams->fmin ) return FALSE;
    return TRUE;
}

//=============================================================================

void horton_initState(THorton *infilParams)
//
//  Input:   infil = ptr. to Horton infiltration object
//  Output:  none
...
void horton_getState(THorton *infilParams, double x[])
{
    x[0] = infilParams->tp;
    x[1] = infilParams->Fe;
}

void horton_setState(THorton *infilParams, double x[])
{
    infilParams->tp = x[0];
    infilParams->Fe = x[1];
}
...
double horton_getInfil(THorton *infilParams, double tstep, double irate, double depth)
//
//  Input:   infil = ptr. to Horton infiltration object
//           tstep =  runoff time step (sec),
...
//
{
    ...
    double FF, FF1, r;
    double f0   = infilParams->f0 * InfilFactor;
    double fmin = infilParams->fmin * InfilFactor;
    double Fmax = infilParams->Fmax;
    double tp   = infilParams->tp;
    double df   = f0 - fmin;
    double kd   = infilParams->decay;
    double kr   = infilParams->regen * Evap.recoveryFactor;
    ...
    if ( fa > ZERO )
    {
        ...
        if ( Fmax > 0.0 )
        {
            if ( infilParams->Fe + fp * tstep > Fmax )
                fp = (Fmax - infilParams->Fe) / tstep;
            fp = MAX(fp, 0.0);
            infilParams->Fe += fp * tstep;
        }
    }
    ...
    else if (kr > 0.0)
    {
        ...
        if ( Fmax > 0.0 )
        {
            infilParams->Fe = fmin*tp + (df/kd)*(1.0 - exp(-kd*tp));
        }
    }
    infilParams->tp = tp;
    return fp;
...
double modHorton_getInfil(THorton *infilParams, double tstep, double irate,
                          double depth)
//
//  Input:   infil = ptr. to Horton infiltration object
//           tstep =  runoff time step (sec),
...
//
{
    ...
    {
        // --- saturated condition
        if ( infilParams->Fmax > 0.0 && infilParams->Fe >= infilParams->Fmax ) return 0.0;

        // --- potential infiltration
        fp = f0 - kd * infilParams->Fe;
        fp = MAX(fp, fmin);
        ...
        infilParams->Fe += MAX((f - fmin), 0.0) * tstep;
        if ( infilParams->Fmax > 0.0 ) infilParams->Fe = MAX(infilParams->Fe, infilParams->Fmax);
    }

    // --- reduce cumulative infiltration for dry condition
    else if (kr > 0.0)
    {
        infilParams->Fe *= exp(-kr * tstep);
        infilParams->Fe = MAX(infilParams->Fe, 0.0);
    }
    return f;
}
...
//
{
    ...
    infilParams->S      = p[0] / UCF(RAINDEPTH);   // Capillary suction head (ft)
    infilParams->Ks     = p[1] / UCF(RAINFALL);    // Sat. hyd. conductivity (ft/sec)
    infilParams->IMDmax = p[2];                    // Max. init. moisture deficit

    // --- find depth of upper soil zone (ft) using Mein's eqn.
    ksat = infilParams->Ks * 12. * 3600.;
    infilParams->Lu = 4.0 * sqrt(ksat) / 12.;
    return TRUE;
}
...
//
{
    if (infilParams == NULL) return;
    infilParams->IMD = infilParams->IMDmax;
    infilParams->Fu = 0.0;
    infilParams->F = 0.0;
    infilParams->Sat = FALSE;
    infilParams->T = 0.0;
}

void grnampt_getState(TGrnAmpt *infilParams, double x[])
{
    x[0] = infilParams->IMD;
    x[1] = infilParams->F;
    x[2] = infilParams->Fu;
...
void grnampt_setState(TGrnAmpt *infilParams, double x[])
{
    infilParams->IMD = x[0];
    infilParams->F   = x[1];
    infilParams->Fu  = x[2];
    infilParams->Sat = (char)x[3];
    infilParams->T   = x[4];
}
...
//
{
    ...
    Fumax = infilParams->IMDmax * infilParams->Lu * sqrt(InfilFactor);

    // --- reduce time until next event
    infilParams->T -= tstep;

    // --- use different procedures depending on upper soil zone saturation
    if ( infilParams->Sat ) return grnampt_getSatInfil(infilParams, tstep, irate, depth);
    else return grnampt_getUnsatInfil(infilParams, tstep, irate, depth, modelType);
}

//=============================================================================

double grnampt_getUnsatInfil(TGrnAmpt *infilParams, double tstep, double irate,
    double depth, int modelType)
//
//  Input:   infil = ptr. to Green-Ampt infiltration object
//           tstep =  runoff time step (sec),
...
//
{
    ...
    double ks = infilParams->Ks * InfilFactor;
    double lu = infilParams->Lu * sqrt(InfilFactor);
    ...
    if ( ia == 0.0 )
    {
        if ( infilParams->Fu <= 0.0 ) return 0.0;
        kr = lu / 90000.0 * Evap.recoveryFactor; 
        dF = kr * Fumax * tstep;
        infilParams->F -= dF;
        infilParams->Fu -= dF;
        if ( infilParams->Fu <= 0.0 )
        {
            infilParams->Fu = 0.0;
            infilParams->F = 0.0;
            infilParams->IMD = infilParams->IMDmax;
        ...
        if ( infilParams->T <= 0.0 )
        {
            infilParams->IMD = (Fumax - infilParams->Fu) / lu; 
            infilParams->F = 0.0;
        }
        return 0.0;
    }
    ...
    if ( ia <= ks )
    {
        ...
        infilParams->F += dF;
        infilParams->Fu += dF;
        infilParams->Fu = MIN(infilParams->Fu, Fumax);
        if ( modelType == GREEN_AMPT &&  infilParams->T <= 0.0 )
        {
            infilParams->IMD = (Fumax - infilParams->Fu) / lu;
            infilParams->F = 0.0;
        }
    ...
    infilParams->T = 5400.0 / lu / Evap.recoveryFactor; 

    // --- find volume needed to saturate surface layer
    Fs = ks * (infilParams->S + depth) * infilParams->IMD / (ia - ks);

    // --- surface layer already saturated
    if ( infilParams->F > Fs )
    {
        infilParams->Sat = TRUE;
        return grnampt_getSatInfil(infilParams, tstep, irate, depth);
    }

    // --- surface layer remains unsaturated
    if ( infilParams->F + ia*tstep < Fs )
    {
        dF = ia * tstep;
        infilParams->F += dF;
        infilParams->Fu += dF;
    ...
    ts  = tstep - (Fs - infilParams->F) / ia;
    if ( ts <= 0.0 ) ts = 0.0;

    // --- compute new total volume infiltrated
    c1 = (infilParams->S + depth) * infilParams->IMD;
...
double grnampt_getSatInfil(TGrnAmpt *infilParams, double tstep, double irate,
    double depth)
//
//  Input:   infil = ptr. to Green-Ampt infiltration object
//           tstep =  runoff time step (sec),
...
//
{
    ...
    c1 = (infilParams->S + depth) * infilParams->IMD;
    F2 = grnampt_getF2(infilParams->F, c1, ks, tstep);
    dF = F2 - infilParams->F;
...
int curvenum_setParams(TCurveNum *infilParams, double p[])
//
...
//
{
    ...
    infilParams->Smax    = (1000.0 / p[0] - 10.0) / 12.0;
    if ( infilParams->Smax < 0.0 ) return FALSE;

    // --- convert drying time (days) to a regeneration const. (1/sec)
    if ( p[2] > 0.0 )  infilParams->regen =  1.0 / (p[2] * SECperDAY);
    else return FALSE;

    // --- compute inter-event time from regeneration const. as in Green-Ampt
    infilParams->Tmax = 0.06 / infilParams->regen;
...
void curvenum_initState(TCurveNum *infilParams)
//
//  Input:   infil = ptr. to Curve Number infiltration object
//  Output:  none
...
//
{
    infilParams->S  = infilParams->Smax;
    infilParams->P  = 0.0;
    infilParams->F  = 0.0;
    infilParams->T  = 0.0;
    infilParams->Se = infilParams->Smax;
    infilParams->f  = 0.0;
}

void curvenum_getState(TCurveNum *infilParams, double x[])
{
    x[0] = infilParams->S;
    x[1] = infilParams->P;
    x[2] = infilParams->F;
...
void curvenum_setState(TCurveNum *infilParams, double x[])
{
    infilParams->S  = x[0];
    infilParams->P  = x[1];
    infilParams->F  = x[2];
...
double curvenum_getInfil(TCurveNum *infilParams, double tstep, double irate,
    double depth)
//
//  Input:   infil = ptr. to Curve Number infiltration object
//           tstep = runoff time step (sec),
...
//           of the ponded depth and not as an effective rainfall rate.
{
    ...
    if ( irate > ZERO )
    {
        ...
        if ( infilParams->T >= infilParams->Tmax )
        {
            infilParams->P = 0.0;
            infilParams->F = 0.0;
            infilParams->f = 0.0;
            infilParams->Se = infilParams->S;
        }
        infilParams->T = 0.0;

        // --- update cumulative precip.
        infilParams->P += irate * tstep;

        // --- find potential new cumulative infiltration
        F1 = infilParams->P * (1.0 - infilParams->P / (infilParams->P + infilParams->Se));

        // --- compute potential infiltration rate
        f1 = (F1 - infilParams->F) / tstep;
        if ( f1 < 0.0 || infilParams->S <= 0.0 ) f1 = 0.0;
    ...
    else
    {
        ...
        if ( depth > MIN_TOTAL_DEPTH && infilParams->S > 0.0 )
        {
            f1 = infilParams->f;
            if ( f1*tstep > infilParams->S ) f1 = infilParams->S / tstep;
        }

        // --- otherwise update inter-event time
        else infilParams->T += tstep;
    }
    ...
    if ( f1 > 0.0 )
    {
        ...
        infilParams->F += f1 * tstep;

        // --- reduce infil. capacity if a regen. constant was supplied
        if ( infilParams->regen > 0.0 )
        {
            infilParams->S -= f1 * tstep;
            if ( infilParams->S < 0.0 ) infilParams->S = 0.0;
        }
    }
    ...
    {
        infilParams->S += infilParams->regen * infilParams->Smax * tstep * Evap.recoveryFactor;
        if ( infilParams->S > infilParams->Smax ) infilParams->S = infilParams->Smax;
    }
    infilParams->f = f1;
    return f1;
}
