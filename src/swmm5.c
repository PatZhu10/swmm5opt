...
//  Purpose: retrieves current value of a subcatchment's property.
{
    ...
    switch (property)
    {
        ...
        case swmm_SUBCATCH_F0:
          return subcatch->infilParams.f0 * UCF(RAINFALL);
        case swmm_SUBCATCH_FMIN:
          return subcatch->infilParams.fmin * UCF(RAINFALL);
        case swmm_SUBCATCH_DECAY:
          return subcatch->infilParams.decay * 3600;
        case swmm_SUBCATCH_REGEN:
          if (subcatch->infilParams.regen == 0)
            return subcatch->infilParams.regen = TINY;
          else
            return -log(1.0-0.98) / (subcatch->infilParams.regen * SECperDAY);
        case swmm_SUBCATCH_FMAX:
          return subcatch->infilParams.Fmax * UCF(RAINDEPTH);
        /*new*/
        default:
          return 0;
    }
